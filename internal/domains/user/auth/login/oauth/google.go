...
func HandleOAuthCallback(w http.ResponseWriter, r *http.Request, staffRepo *staffs.Repository) {
	...
	signedToken, err := auth.SignJWT(*userInfo)
	if err != nil {
		utils.RespondWithError(w, err)
		return
	}
...
func getUserInfo(accessToken string, staffRepo *staffs.Repository, c context.Context) (*auth.UserInfo, *types.HTTPError) {
