...
const deletePendingStaff = `-- name: DeletePendingStaff :exec
DELETE FROM staff.pending_staff
WHERE id = $1
`

func (q *Queries) DeletePendingStaff(ctx context.Context, id uuid.UUID) error {
	_, err := q.db.ExecContext(ctx, deletePendingStaff, id)
	return err
}
