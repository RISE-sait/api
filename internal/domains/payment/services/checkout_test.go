...
func TestCreateSubscription(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			periods:       12,
			wantErr:       true,
			errMsg:        "must be one of month, year, week, or day",
		},
		{
			name:          "yearly subscription",
			planName:      "Annual Plan",
			price:         decimal.NewFromFloat(99.99),
			interval:      payment.IntervalYear,
			intervalCount: 1,
			periods:       1,
			wantErr:       true,
			errMsg:        "totalBillingPeriods must be at least 2 for subscriptions. Use create one time payment if its not recurring",
		},
		{
