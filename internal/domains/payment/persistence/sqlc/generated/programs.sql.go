...
const getProgramsTeam = `-- name: GetProgramsTeam :one
SELECT t.id
FROM events.events e
         JOIN athletic.teams t ON e.team_id = t.id
WHERE e.program_id = $1
`

func (q *Queries) GetProgramsTeam(ctx context.Context, programID uuid.NullUUID) (uuid.UUID, error) {
	row := q.db.QueryRowContext(ctx, getProgramsTeam, programID)
	var id uuid.UUID
	err := row.Scan(&id)
	return id, err
}
