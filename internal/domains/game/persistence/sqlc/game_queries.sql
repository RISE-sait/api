...
JOIN athletic.teams at ON g.away_team_id = at.id
JOIN location.locations loc ON g.location_id = loc.id
WHERE g.home_team_id = sqlc.arg('team_id') OR g.away_team_id = sqlc.narg('team_id')
ORDER BY g.start_time ASC
LIMIT sqlc.arg('limit') OFFSET sqlc.arg('offset');

-- name: GetUpcomingGamesByTeamID :many
SELECT
    g.id,
    g.home_team_id,
    ht.name AS home_team_name,
...
JOIN athletic.teams at ON g.away_team_id = at.id
JOIN location.locations loc ON g.location_id = loc.id
WHERE (g.home_team_id = sqlc.arg('team_id') OR g.away_team_id = sqlc.narg('team_id')) AND g.end_time >= NOW()
ORDER BY g.start_time ASC
LIMIT sqlc.arg('limit') OFFSET sqlc.arg('offset');

-- name: GetPastGamesByTeamID :many
SELECT
    g.id,
    g.home_team_id,
    ht.name AS home_team_name,
...
JOIN athletic.teams at ON g.away_team_id = at.id
JOIN location.locations loc ON g.location_id = loc.id
WHERE (g.home_team_id = sqlc.arg('team_id') OR g.away_team_id = sqlc.narg('team_id')) AND g.start_time < NOW()
ORDER BY g.start_time DESC
LIMIT sqlc.arg('limit') OFFSET sqlc.arg('offset');