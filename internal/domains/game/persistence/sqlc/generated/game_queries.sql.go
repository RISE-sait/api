...
const getLiveGamesByTeams = `-- name: GetLiveGamesByTeams :many
SELECT
    g.id,
    g.home_team_id,
    ht.name AS home_team_name,
...
type GetLiveGamesByTeamsParams struct {
	TeamIds []uuid.UUID `json:"team_ids"`
	Offset  int32       `json:"offset"`
	Limit   int32       `json:"limit"`
}

type GetLiveGamesByTeamsRow struct {
	ID              uuid.UUID      `json:"id"`
	HomeTeamID      uuid.UUID      `json:"home_team_id"`
	HomeTeamName    string         `json:"home_team_name"`
	HomeTeamLogoUrl sql.NullString `json:"home_team_logo_url"`
...
func (q *Queries) GetLiveGamesByTeams(ctx context.Context, arg GetLiveGamesByTeamsParams) ([]GetLiveGamesByTeamsRow, error) {
	rows, err := q.db.QueryContext(ctx, getLiveGamesByTeams, pq.Array(arg.TeamIds), arg.Offset, arg.Limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetLiveGamesByTeamsRow
	for rows.Next() {
		var i GetLiveGamesByTeamsRow
		if err := rows.Scan(
			&i.ID,
			&i.HomeTeamID,
			&i.HomeTeamName,
