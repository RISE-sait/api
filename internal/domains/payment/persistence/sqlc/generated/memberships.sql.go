...
const getMembershipPlanAmtPeriods = `-- name: GetMembershipPlanAmtPeriods :one
SELECT amt_periods
FROM membership.membership_plans
WHERE id = $1
`

func (q *Queries) GetMembershipPlanAmtPeriods(ctx context.Context, id uuid.UUID) (sql.NullInt32, error) {
	row := q.db.QueryRowContext(ctx, getMembershipPlanAmtPeriods, id)
	var amt_periods sql.NullInt32
	err := row.Scan(&amt_periods)
	return amt_periods, err
}
